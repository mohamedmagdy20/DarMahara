...
<div class="page-content">
  ...
  <div class="col-12">
      <div class="card">
          <div class="card-body">
              ...
              <form method="post" action="{{ route('admin.articles.update') }}"  class="needs-validation" enctype="multipart/form-data"  novalidate >
                  @csrf
                  <input type="hidden" name="id" value="{{$article->id}}">
              <div class="row mb-3">
                  <label for="example-text-input" class="col-sm-1 col-form-label">اسم المقال</label>
                  <div class="col-sm-11">
                      <input name="name" class="form-control" type="text" id="example-text-input" value="{{$article->name}}" required>
                      @error('name')
            ...
            <div class="row mb-3">
              ...
              <div class="col-sm-11">
                  <input name="image_alt"  class="form-control" type="text" id="image_alt" value="{{$article->image_alt}}"  required>
                  @error('image_alt')
                  <span class="text-danger"> {{ $message }} </span>
                  @enderror
              </div>
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-11">
              <textarea name="description" id="text-area" cols="30" rows="10">{!! $article->description !!}</textarea>
                @error('description')
                <span class="text-danger"> {{ $message }} </span>
                @enderror
            </div>
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-11">
           <input type="text" name="short_description" id="short_decs" value="{{$article->short_description}}"  class="form-control">
      ...
      <div class="row mb-3">
        ...
        <div class="col-sm-11">
         <input type="text" name="meta_title"  id="meta_title" value="{{$article->meta_title}}" class="form-control">
            @error('meta_title')
            <span class="text-danger"> {{ $message }} </span>
            @enderror
        </div>
  ...
  <div class="row mb-3">
    ...
    <div class="col-sm-11">
     <input type="text" autocomplete="off"  name="keywords" value="{{$article->keywords}}" id="input-tags"  class="form-control" >
        @error('keywords')
        <span class="text-danger"> {{ $message }} </span>
        @enderror
    </div>
    ...
    <div class="row mb-3">
      ...
      <div class="col-sm-11">
       <input type="url" name="url"  id="url" value="{{$article->url}}" class="form-control" >
          @error('url')
          <span class="text-danger"> {{ $message }} </span>
          @enderror
      </div>
